...
<head>
    ...
    <title>@yield('title', 'KanbanFlow') — KanbanFlow</title>

    {{-- Google Fonts --}}
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600&family=Syne:wght@700;800&display=swap" rel="stylesheet">

    {{-- Tailwind CSS CDN (remplacer par Vite en prod) --}}
...
<nav class="navbar sticky top-0 z-40">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 h-[65px] flex items-center gap-6">
        ...
        {{-- Liens de navigation (visibles si connecté) --}}
        @auth
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="relative" id="user-menu">
                <button onclick="toggleDropdown()"
                        class="flex items-center gap-2 rounded-xl px-2 py-1 hover:bg-ink-100 transition-colors"
                        style="border:none;background:none;cursor:pointer;">
                    <div class="avatar">
                        {{ strtoupper(substr(auth()->user()->name, 0, 2)) }}
                    </div>
                    <span class="text-sm font-medium text-ink-800 hidden sm:block">
                        {{ auth()->user()->name }}
                    </span>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6b6d8a" stroke-width="2" stroke-linecap="round">
                        <polyline points="6 9 12 15 18 9"/>
                    </svg>
                </button>

                <div class="dropdown-menu" id="dropdown-menu">
                    <div style="padding:0.75rem 1rem;border-bottom:1px solid #e8e9f2;">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('register') }}"
               class="text-sm font-semibold px-4 py-2 rounded-xl text-white transition-all"
               style="background:#4f6ef7;text-decoration:none;"
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 pt-4">
    @if(session('success'))
    <div class="flash flex items-center gap-3 px-4 py-3 rounded-xl mb-3"
         style="background:#f0fdf4;border:1px solid #86efac;color:#166534;">
        <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <polyline points="20 6 9 17 4 12"/>
        </svg>
        <span class="text-sm font-medium">{{ session('success') }}</span>
        <button onclick="this.parentElement.remove()" class="ml-auto text-green-400 hover:text-green-600" style="border:none;background:none;cursor:pointer;">✕</button>
    </div>
    @endif
    ...
    <div class="flash flex items-center gap-3 px-4 py-3 rounded-xl mb-3"
        ...
        <button onclick="this.parentElement.remove()" class="ml-auto text-red-400 hover:text-red-600" style="border:none;background:none;cursor:pointer;">✕</button>
    </div>
    @endif
    ...
    <div class="flash flex items-center gap-3 px-4 py-3 rounded-xl mb-3"
        ...
        <button onclick="this.parentElement.remove()" class="ml-auto text-yellow-500 hover:text-yellow-700" style="border:none;background:none;cursor:pointer;">✕</button>
    </div>
    @endif
</div>
@endif
...
{{-- ============================================================
     CONTENU PRINCIPAL
     Chaque page fait @extends('layouts.app') et @section('content')
     ============================================================ --}}
<main>
    @yield('content')
</main>
...
<script>
    ...
    // Auto-fermeture des flash messages après 4s
    setTimeout(() => {
